<template>
  <div class="min-h-screen bg-slate-900 text-slate-200 font-sans">
    <nav v-if="isAuthenticated && !['/setup', '/login'].includes($route.path)"
      class="border-b border-slate-800 bg-slate-900/50 backdrop-blur-xl sticky top-0 z-50">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16 items-center">
          <div class="flex items-center space-x-3">
            <img src="/icon.png" alt="PM2Me" class="w-8 h-8 rounded-lg shadow-lg shadow-blue-500/30 cursor-pointer"
              @click="$router.push('/')" />
            <span
          ...
          <div class="flex items-center space-x-1 sm:space-x-2">
            ...
            <button @click="$router.push('/settings')" :class="[
              'flex items-center space-x-2 px-3 py-2 rounded-xl text-sm font-medium transition-all duration-200',
              $route.path === '/nginx'
                ? 'bg-blue-600/10 text-blue-400 shadow-sm border border-blue-500/20'
                : 'text-slate-400 hover:text-white hover:bg-slate-800'
