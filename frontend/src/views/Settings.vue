  ...
  <div class="max-w-4xl mx-auto px-4 py-8">
      ...
      <div class="p-6 space-y-6">
        ...
        <div>
          <div class="flex items-center justify-between mb-2">
            ...
            <button @click="copyToClipboard(webhookUrl, 'URL')" class="text-xs text-blue-400 hover:text-blue-300 flex items-center transition-colors">
          ...
          <div class="relative">
            <input type="text" :value="webhookUrl" readonly class="w-full bg-slate-900/50 border border-slate-700 rounded-lg px-4 py-2.5 text-slate-400 text-sm font-mono outline-none transition-all cursor-default" placeholder="Calculating...">
          </div>
          <p class="mt-1 text-[11px] text-slate-500 italic">Provide this URL to your GitHub repository's Webhook settings.</p>
        </div>
        ...
        <div>
          <div class="flex items-center justify-between mb-2">
            ...
            <div class="flex items-center space-x-3">
              <button @click="generateSecret" class="text-xs text-emerald-400 hover:text-emerald-300 flex items-center transition-colors">
                <svg class="w-3.5 h-3.5 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                </svg>
                Generate
              </button>
              <button @click="copyToClipboard(settings.webhookSecret, 'Secret')" class="text-xs text-blue-400 hover:text-blue-300 flex items-center transition-colors">
                <svg class="w-3.5 h-3.5 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-1M8 5a2 2 0 002 2h2a2 2 0 002-2M8 5a2 2 0 012-2h2a2 2 0 012 2m0 0h2a2 2 0 012 2v3m2 4H10m0 0l3-3m-3 3l3 3" />
                </svg>
          ...
          <div class="mt-4 p-4 bg-blue-500/5 border border-blue-500/20 rounded-xl space-y-3">
            ...
            <ol class="text-xs text-slate-400 space-y-2 list-decimal list-inside">
              ...
              <li>Go to <span class="text-slate-200">Settings</span> > <span class="text-slate-200">Webhooks</span> > <span class="text-slate-200">Add webhook</span>.</li>
            ...
            <p class="text-[10px] text-slate-500 italic">This allows PM2Me to automatically deploy your apps when you push code.</p>
          </div>
        </div>
    ...
    <div class="mt-8 bg-slate-800/80 backdrop-blur-sm rounded-2xl shadow-xl border border-slate-700 overflow-hidden">
      <div class="p-6 border-b border-slate-700 flex items-center justify-between">
        ...
        <button @click="fetchWebhookLogs" class="p-2 text-slate-400 hover:text-white transition-colors" title="Refresh Logs">
          <svg class="w-5 h-5" :class="{ 'animate-spin': isFetchingLogs }" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
          </svg>
        </button>
      </div>
